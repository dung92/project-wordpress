<p>
	<b><?php _e( 'Transaction Status', 'woo-payment-gateway' ); ?>:</b>&nbsp;<?php echo ucfirst( str_replace( '_', ' ', $status ) ); ?></p>
<?php
if ( $status === \Braintree\Transaction::AUTHORIZED && $order->get_meta( '_authorization_exp_at' ) ) :
	$date = new DateTime( '@' . $order->get_meta( '_authorization_exp_at' ) );
	?>
<p>
	<b><?php _e( 'Authorization Expires', 'woo-payment-gateway' ); ?>:</b>&nbsp;<?php echo date_format( $date, 'M d Y, h:i A e' ); ?>
<?php endif; ?>
<?php
