...
class WC_Braintree_PayPal_Payment_Gateway extends WC_Braintree_Payment_Gateway {
				...
				array(
					...
					'value' => array(
						...
						'breakdown'     => array(
							'item_total'        => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( $incl_tax ? WC()->cart->subtotal + WC()->cart->fee_total + get_fee_tax() : WC()->cart->subtotal_ex_tax + WC()->cart->fee_total, $decimals ), $decimals )
							),
							'shipping'          => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( $incl_tax ? WC()->cart->shipping_total + WC()->cart->shipping_tax_total : WC()->cart->shipping_total, $decimals ), $decimals )
							),
							'tax_total'         => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( $incl_tax ? 0 : WC()->cart->get_taxes_total(), $decimals ), $decimals )
							),
							'shipping_discount' => array(
								'currency_code' => $currency,
								'value'         => wc_format_decimal( round( $incl_tax ? WC()->cart->discount_cart + WC()->cart->discount_cart_tax : WC()->cart->discount_cart, $decimals ), $decimals )
							)
						)
					)
				),
	...
	public function get_formatted_shipping_method( $method, $index, $selected, $amount, $decimals ) {
		...
		return array(
			...
			'selected' => $selected,
			'amount'   => array(
				'value'         => wc_format_decimal( round( $amount, $decimals ), $decimals ),
