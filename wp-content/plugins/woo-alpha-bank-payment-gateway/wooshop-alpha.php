                ...
                $doseis_field = '<p class="form-row ">
                    <label for="' . esc_attr($this->id) . '-card-doseis">' . __('Choose Installments', 'woo-alpha-bank-payment-gateway') . ' <span class="required">*</span></label>
                        ...
                        for ($i = 1; $i <= $max_installments; $i++) {
                            $doseis_field .= '<option value="' . $i . '">' . ($i == 1 ? __('Without installments','woo-payment-gateway-for-eurobank') : $i). '</option>';
                        }
                        $doseis_field .= '</select>
                ...
                if($order->get_status() == 'processing') {
                    ...
                    if ($order->has_downloadable_item()) {
                        ...
                        if($hasOnlyDownloadable) {
                            //Update order status
                            $order->update_status('completed', __('Payment received, your order is now complete.', 'woo-alpha-bank-payment-gateway'));

                            //Add admin order note
                            $order->add_order_note(__('Payment Via Alpha Bank<br />Transaction ID: ', 'woo-alpha-bank-payment-gateway') . $paymentRef);

                            //Add customer order note
                            $order->add_order_note(__('Payment Received.<br />Your order is now complete.<br />Alpha Transaction ID: ', 'woo-alpha-bank-payment-gateway') . $paymentRef, 1);
                            ...
                            //Update order status
                            $order->update_status('processing', __('Payment received, your order is currently being processed.', 'woo-alpha-bank-payment-gateway'));
