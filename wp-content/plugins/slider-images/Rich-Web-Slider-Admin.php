<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
	{
		if(check_admin_referer( 'edit-menu_', 'Rich_Web_PSlider_Nonce' ))
		{
			...
			$SL_Img_Title = $Sl_Img_Description = $Sl_Img_Url = $Sl_Link_Url = 	$Sl_Link_NewTab = array();
	...
	$RW_Slider_New = (empty($jaboId)) ?  1 :  $jaboId[0]->Slider_ID + 1;
?>
<form method="POST" enctype="multipart/form-data">
	...
	<div class="RW_IS_Buttons" style="position: relative; width: 100%; right: 1%; height: 50px;">
		...
		<input type='button' class='JAddSlider' value='New Slider' onclick='addSliderJ(<?php echo $RW_Slider_New;?>)' />
	...
	<div class="Rich_Web_SliderIm_Absolute_Div">
		...
		<i class="Rich_Web_IS_Del_Trash rich_web rich_web-trash"></i>
	...
	<div class='Table_Data_rich_web_Content' >
		<div class='Table_Data_rich_web1'>
			...
			<?php for($i=0;$i<count($Rich_WebDat);$i++){?> 
				<tr class='rich_web_Tit_Table_Tr2 RW_IS_Table_Tr_<?php echo $Rich_WebDat[$i]->id;?>'>
					<td><?php echo $i+1; ?></td>
					<td><?php echo $Rich_WebDat[$i]->Slider_Title; ?></td>
				<?php 	$Rich_Web_IS_Opt_Name = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE id=%s", (int) $Rich_WebDat[$i]->Slider_Type)); ?>
					<td><?php echo $Rich_Web_IS_Opt_Name[0]->slider_name; ?></td>
		...
		<div class='Table_Data_rich_web2'>
			<table class="rich_web_ShortTable" style="display: table; float: right;">
				...
				<tr>
					<td class="rich_web_Slider_ID"  unselectable="on" onselectstart="return false;"  onmousedown="return false;" onclick="rw_is_copy(this)" onmouseout="rw_is_copied(this)" >
				...
				<tr>
					<td class="JMBSL"  unselectable="on" onselectstart="return false;"  onmousedown="return false;"  onclick="rw_is_copy(this)" onmouseout="rw_is_copied(this)" >&lt;?php echo do_shortcode('[Rich_Web_Slider id="1"]');?&gt;</td>
				</tr>
			</table>
					...
					<td colspan='2'>
						<select class='JSlInput JSType' name='Slider_Type'>
							<?php for($i=0;$i<count($Rich_WebEff);$i++){ ?>
								<option value='<?php echo $Rich_WebEff[$i]->id;?>'><?php echo $Rich_WebEff[$i]->slider_name;?></option>
							<?php }?>
						</select>
					</td>
				</tr>
