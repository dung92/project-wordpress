<?php
	...
	function rich_web_Copy_Sl_Callback()
	{
		...
		$table_name_effect = $wpdb->prefix . "rich_web_slider_effects_data";
		$Rich_Web_Manager=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE id=%s", $number));
		$Rich_Web_Instal=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name1 WHERE Sl_Number=%d order by id", $number));
		...
		$New_Slider_ID = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name order by id desc"));
		$New_Slider_Type = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name_effect where id=%s",$New_Slider_ID[0]->Slider_Type));
		$New_Slider_ID[0]->Slider_Type = $New_Slider_Type[0]->slider_name;
		echo json_encode($New_Slider_ID[0]);
		die();
	}
	...
	function rich_web_Del_Option_Callback()
	{
		...
		$rw_is_opt_type_exp = explode(" ",$Rich_Web_Effects[0]->slider_type);
		($rw_is_opt_type_exp[0] == "Slider") ? $rw_is_opt_type = $rw_is_opt_type_exp[1] : $rw_is_opt_type = $rw_is_opt_type_exp[0];
		$RW_IS_Checking=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE slider_type=%s", $Rich_Web_Effects[0]->slider_type));
		...
		$query_array = explode(",",$RW_IS_queries_arr[$Rich_Web_Effects[0]->slider_type]);
		$RW_IS_Q_Table = $wpdb->prefix  . $query_array[0];
		$RW_IS_Load_Q_Table = $wpdb->prefix  . $query_array[0] . "_Loader";
		$wpdb->query($wpdb->prepare("DELETE FROM $RW_IS_Q_Table WHERE $query_array[1] = %s", $Rich_Web_Slider_ID));
		$wpdb->query($wpdb->prepare("DELETE FROM $RW_IS_Load_Q_Table WHERE $query_array[2] = %s", $Rich_Web_Slider_ID));
		if (count($RW_IS_Checking) == 0) {
			$file_php = plugin_dir_path( __FILE__ ) . "PHP/RW_Img_Slider_".$rw_is_opt_type.".php";
			$file_js = plugin_dir_path( __FILE__ ) . "Scripts/RW_Img_Slider_".$rw_is_opt_type.".js.php";
			$file_css = plugin_dir_path( __FILE__ ) . "Style/RW_Img_Slider_".$rw_is_opt_type.".css.php";
			unlink($file_php);
			unlink($file_js);
			unlink($file_css);
			$wpdb->query( "DROP TABLE IF EXISTS $RW_IS_Q_Table" );
		}
		die();
	}
	...
	function rich_web_Edit_Option_Callback()
	{
		...
		$RW_IS_Q_Table = $wpdb->prefix  . $query_array[0];
		$RW_IS_Load_Q_Table = $wpdb->prefix  . $query_array[0] . "_Loader";
		$Rich_Web_Effect=$wpdb->get_results($wpdb->prepare("SELECT * FROM $RW_IS_Q_Table WHERE $query_array[1] = %s ",$Rich_Web_Slider_ID));
		$Rich_Web_Effect_Loading=$wpdb->get_results($wpdb->prepare("SELECT * FROM $RW_IS_Load_Q_Table WHERE $query_array[2] = %s", $Rich_Web_Slider_ID));
	...
	function rich_web_Copy_Sl2_Callback()
	{
		...
		$Rich_Web_Effects=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE id=%d", $Rich_Web_Slider_ID));
		$Rich_Web_IS_Opt_Name = $Rich_Web_Effects[0]->slider_name;
		for ($i=1; $i < 100; $i++) { 
			$Rich_Web_IS_New_Opt_Name = $Rich_Web_IS_Opt_Name.' '.$i;
			$Check_RW_IS_Opt_Name =	 $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE slider_name = %s", $Rich_Web_IS_New_Opt_Name));
			if(count($Check_RW_IS_Opt_Name) == 0){
				$Rich_Web_Effects[0]->slider_name = $Rich_Web_IS_New_Opt_Name;
				break;
			}
		}
		...
		$Rich_Web_Slider_New_ID[0]->copiedOptionID = $Rich_Web_Slider_ID;
		echo json_encode($Rich_Web_Slider_New_ID[0]);
		die();
	}

	add_action( 'wp_ajax_Rich_Web_IS_Insert_Option', 'Rich_Web_IS_Insert_Option_Callback' );
	add_action( 'wp_ajax_nopriv_Rich_Web_IS_Insert_Option', 'Rich_Web_IS_Insert_Option_Callback' );
	function Rich_Web_IS_Insert_Option_Callback()
	{
		/* global $wpdb; */
		...
		$Option_Type = sanitize_text_field($_POST['Type']);
		//fileNames
		$filename = plugin_dir_path( __FILE__ ) . 'PHP/RW_Img_Slider_'.$Option_Type.'.php';
		$filename_js = plugin_dir_path( __FILE__ ) . 'Scripts/RW_Img_Slider_'.$Option_Type.'.js.php';
		$filename_css = plugin_dir_path( __FILE__ ) . 'Style/RW_Img_Slider_'.$Option_Type.'.css.php';
		//Dir URI
		$dir_name =  plugin_dir_path( __FILE__ );
		//Moves URLs
		$move_from = plugin_dir_path( __FILE__ ) . "RW_Img_Slider_".$Option_Type."/RW_Img_Slider_".$Option_Type.".php";
		$move_to = plugin_dir_path( __FILE__ ) . "PHP/RW_Img_Slider_".$Option_Type.".php";
		$move_from_js = plugin_dir_path( __FILE__ ) . "RW_Img_Slider_".$Option_Type."/RW_Img_Slider_".$Option_Type.".js.php";
		$move_to_js = plugin_dir_path( __FILE__ ) . "Scripts/RW_Img_Slider_".$Option_Type.".js.php";
		$move_from_css = plugin_dir_path( __FILE__ ) . "RW_Img_Slider_".$Option_Type."/RW_Img_Slider_".$Option_Type.".css.php";
		$move_to_css = plugin_dir_path( __FILE__ ) . "Style/RW_Img_Slider_".$Option_Type.".css.php";
		$move_from_query = plugin_dir_path( __FILE__ ) . "RW_Img_Slider_".$Option_Type."/RW_IS_".$Option_Type."_Query.php";
		$move_to_query = plugin_dir_path( __FILE__ ) . "RW_IS_".$Option_Type."_Query.php";
		//Folder Name
		$folderName = plugin_dir_path( __FILE__ ) . "RW_Img_Slider_".$Option_Type;
		$ch = curl_init();
		$source = "https://rich-web.org/is.org-free/RW_Img_Slider_".$Option_Type.".zip";
		curl_setopt($ch, CURLOPT_URL, $source);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$data = curl_exec ($ch);
		curl_close ($ch);
		$destination = "RW_Img_Slider_".$Option_Type.".zip"; 
		$file = fopen($destination, "w+");
		fputs($file, $data);
		fclose($file);
		$zip = new ZipArchive;
		$res = $zip->open("RW_Img_Slider_".$Option_Type.".zip");
		if ($res === TRUE) {
			$zip->extractTo($dir_name); // directory to extract contents to
			$zip->close();
			unlink("RW_Img_Slider_".$Option_Type.".zip");
		}
		...
		while($file = readdir($folderHandle)) {
			if ($file != "." && $file != "..") {
			  ...
			  rmdir($folderName);
			  require_once( "RW_IS_".$Option_Type."_Query.php ");
			  RW_IS_Query();  
			  $remove_file = plugin_dir_path( __FILE__ ) ."RW_IS_".$Option_Type."_Query.php";
			  unlink($remove_file);
			  global $wpdb;
			  $table_name2  = $wpdb->prefix . "rich_web_slider_effects_data";
			  $Rich_Web_Slider_ID=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE id > %d order by id desc limit 1", 0));
			  echo json_encode($Rich_Web_Slider_ID[0]);
			die();
	}

	add_action( 'wp_ajax_Rich_Web_IS_Name_Available', 'Rich_Web_IS_Name_Available_Callback' );
	add_action( 'wp_ajax_nopriv_Rich_Web_IS_Name_Available', 'Rich_Web_IS_Name_Available_Callback' );
	function Rich_Web_IS_Name_Available_Callback(){
		global $wpdb;
		$table_name_effect = $wpdb->prefix . "rich_web_slider_effects_data";
		
		$RW_IS_name = sanitize_text_field($_POST['RW_IS_name']);
		$RW_IS_id = sanitize_text_field($_POST['RW_IS_id']);

		if ($RW_IS_id == 'none') {
			$RW_IS_Name_Avaible=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name_effect WHERE slider_name = %s", $RW_IS_name));
			if (count($RW_IS_Name_Avaible) == 0) {
				echo('true');
			}else{
				echo("false");
			}
		}else{
			$RW_IS_Name_Avaible=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name_effect WHERE id != %s AND slider_name = %s", $RW_IS_id, $RW_IS_name));
			if (count($RW_IS_Name_Avaible) == 0) {
				echo('true');
			}else{
				echo("false");
