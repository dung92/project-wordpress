...
class GuestBackgroundConverter {
	...
	public function convert_optins_and_queue_next( $page ) {
		...
		try {
			...
			$optin_repository = new EmailOptRepository( $wpdb );
			$emails           = $optin_repository->get_all_optin_emails( $page, 10 );
			foreach ( $emails as $email ) {
				$this->logger->debug( "Guest extraction from email: {$email}" );
			...
			if ( count( $emails ) > 0 ) {
				$this->queue_client->add( self::HOOK_CONVERT_OPTIN_PAGE, [
					'page' => $page + 1
				] );
			}
		} catch ( \Throwable $e ) {
			$this->logger->error( "Error while converting optin to guest", [
				'exception' => $e,
				'guest'     => $guest
			] );
		}
	...
	public function convert_orders_and_queue_next( $page ) {
		...
		try {
			...
			if ( count( $orders->orders ) > 0 ) {
				$this->queue_client->add( self::HOOK_CONVERT_ORDER_PAGE, [
					'page' => $page + 1
				] );
			} else { // after all orders we can convert outcomes and optins
				$this->queue_client->add( self::HOOK_CONVERT_OUTCOME_PAGE, [
					'page' => 0
				] );
				$this->queue_client->add( self::HOOK_CONVERT_OPTIN_PAGE, [
					'page' => 0
			...
			$this->logger->error( "Error while converting order to guest", [
				'exception' => $e,
	...
	public function convert_outcomes_and_queue_next( $page ) {
		...
		try {
			...
			$this->logger->error( "Error while injectings guests to outcomes", [
				'exception' => $e
			] );
		}
	}
