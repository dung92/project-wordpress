...
/**
 * Kind of repositoruy pattern with some methods to insert outcomes.
 *
 * @package WPDesk\ShopMagic\AutomationOutcome
 */
final class OutcomeReposistory implements OutcomeInformationRepository {
	...
	public function log_note( string $execution_id, string $note, array $context = [] ) {
		...
		$this->wpdb->update( $outcome_table, [
	 ...
	 * @param string $note
	 * @param array $context
	 * @param int $now Time in unix UTC
	 */
	 ...
	 *
	 * @return string|int Unique outcome id to log future notices and savr finish.
	 */
	public function prepare_for_outcome( Automation $automation, Event $event, Action $action, $action_index ) {
		...
		$outcome_table = DatabaseSchema::get_automation_outcome_table_name();
		$this->wpdb->insert( $outcome_table, [
	 ...
	 * @return array
	 */
	private function inject_guest_support( array $filter ) {
	 ...
	 * @param array $item item from database.
	 *
	 * @return Customer
	 * @throws \Exception
	 */
	private function create_customer_from_outcome_row( CustomerFactory $customer_factory, array $item ) {
