...
final class TableList extends \WP_List_Table {
	...
	private function prepare_search_args() {
		$search_args = [
			'group'  => QueueExecutionStrategyFactory::QUEUE_GROUP,
	...
	public function prepare_items() {
		...
		$items = $this->queue->search( array_merge( $search_args, [
			'per_page' => $page_size,
			'offset'   => $offset,
			'orderby'  => 'date',
	...
	protected function column_options( $as_action ): string {
		return sprintf( __( '<a class="cancel_queue" data-sure="%s" href="%s">Cancel</a>', 'shopmagic-for-woocommerce' ),
			sprintf( __( 'Are you sure you want to cancel queued automation %s?', 'shopmagic-for-woocommerce' ), $this->column_id( $as_action ) ),
