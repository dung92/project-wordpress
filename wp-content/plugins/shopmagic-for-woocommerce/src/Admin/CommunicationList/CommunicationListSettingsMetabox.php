...
use WPDesk\ShopMagic\FormField\Field\Header;
...
final class CommunicationListSettingsMetabox implements FieldProvider {
	...
	public function get_fields() {
		return [
			( new SelectField() )
				...
				->set_description( sprintf( __( 'Opt-in communication requires customer consent. Opt-out communication (not recommended) is sent until the customer opts out. %sLearn more &rarr;%s', 'shopmagic-for-woocommerce' ), '<a href="https://docs.shopmagic.app/" target="_blank">', '</a>' ) ),
			( new CheckboxField() )
	...
	public function save_fields( $post_id ) {
		if ( isset( $_POST['post_type'] ) && $_POST['post_type'] === CommunicationListPostType::TYPE ) {
	...
	public function hooks() {
		add_meta_box( 'shopmagic_list_settings_metabox', __( 'Settings', ' shopmagic-for-woocommerce' ), array(
			$this,
			'render_list_fields'
		), CommunicationListPostType::TYPE, 'normal' );
