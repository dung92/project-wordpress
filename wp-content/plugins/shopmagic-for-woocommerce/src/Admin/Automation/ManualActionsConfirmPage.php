...
final class ManualActionsConfirmPage {
	...
	public function hooks() {
		 ...
		 */
		add_filter( 'duplicate_post_excludelist_filter', static function ( $meta_excludelist ) {
	...
	private function queue_for_trigger_matched_items( Automation $automation, array $ids ) {
		...
		do {
			...
			$queue->add( ManualActionsTriggerQueue::HOOK_TRIGGER_FOR_SLICE, [
				$automation->get_id(),
				$slice
			] );
	...
	public function render_manual_actions() {
		...
		if ( $never_run_before && isset( $_POST['run'] ) ) {
		...
		if ( $never_run_before ) {
			if ( $event instanceof RenderableItemWithPaginationProvider ) {
				echo $renderer->render( 'manual_actions_confirm_queued', [
				...
				echo $renderer->render( 'manual_actions_confirm', [
					'automation'              => $automation,
					'item_renderer'           => $event,
					'matched_items_generator' => $event->get_items(),
					'renderer'                => $renderer
			...
			echo $renderer->render( 'manual_actions_done', [
				'automation' => $automation
			] );
		}
	}
