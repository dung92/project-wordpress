...
final class TwoWeeksNotice {
	 ...
	 * @param Automation[] Required to check if at least one fully configured automation exists.
	...
	public function hooks() {
		add_action( 'admin_enqueue_scripts', function () {
			wp_enqueue_script( 'shopmagic-rate-notice', $this->assets_url . '/js/two-weeks-notice.js' );
		} );
		add_action( 'wp_ajax_shopmagic_close_temporary', function () {
	...
	private function action_links() {
		$actions[] = sprintf(
			__( '%1$sOk, you deserved it%2$s', 'shopmagic-for-woocommerce' ),
			'<a target="_blank" href="' . esc_url( 'https://wpde.sk/sm-rate' ) . '">',
			'</a>'
		);
		$actions[] = sprintf(
			__( '%1$sNope, maybe later%2$s', 'shopmagic-for-woocommerce' ),
			'<a data-type="date" class="sm-close-temporary-notice" data-source="' . self::CLOSE_TEMPORARY_NOTICE . '" href="#">',
			'</a>'
		);
		$actions[] = sprintf(
			__( '%1$sI already did%2$s', 'shopmagic-for-woocommerce' ),
	...
	private function is_fully_configured_automation_exists() {
		return count( array_filter( $this->automations, function ( Automation $automation ) {
	...
	private function get_message() {
		$message = __( 'Awesome, you\'ve been using ShopMagic for more than 2 weeks. Could you please do me a BIG favor and give it a 5-star rating on WordPress.org? ~ Mac @ ShopMagic Team',
