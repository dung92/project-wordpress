...
class ExecuteNow implements ExecutionStrategy {
	...
	public function execute( Automation $automation, Event $event, Action $action, $action_index, $unique_id ) {
		...
		if ( $event instanceof SupportsDeferredCheck && ! $event->is_event_still_valid() ) {
			LoggerFactory::get_logger()->notice( "Deferred check. Event: {$event->get_name()}. Automation: {$automation->get_id()}. Order linked to Event has changed status again and is no longer consistent with this event.",
				[
			...
			throw new ActionDisabledAfterStatusRecheckException( __( 'Order linked to Event has changed status again and is no longer consistent with this event', 'shopmagic-for-woocommerce' ) );
		}
		...
		try {
			...
			$this->save_outcome( $unique_id, false, "error: {$e->getMessage()}", [
