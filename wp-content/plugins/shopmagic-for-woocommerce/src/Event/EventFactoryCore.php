...
class EventFactoryCore implements EventFactory2 {
	...
	public function create_event( string $slug, Automation $automation, FilterLogic $filters ): Event {
		$event = clone $this->get_event( $slug );
	...
	public function get_event_list(): array {
		if ( empty( self::$events_cache ) ) {
			self::$events_cache = apply_filters( 'shopmagic/core/events', array_merge( $this->get_build_in_events(), $this->get_not_built_in_events() ) );
		}
	 ...
	 * @return Event[]
	 */
	private function get_build_in_events(): array {
		$events = [
