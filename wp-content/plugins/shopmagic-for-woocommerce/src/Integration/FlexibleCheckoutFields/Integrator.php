...
final class Integrator {
	public function integrate_placeholders( PlaceholderFactory2 $factory, LoggerInterface $logger ) {
		add_action( 'flexible_checkout_fields/init', static function ( $integrator ) use ( $factory, $logger ) {
			if ( $integrator instanceof \WPDesk\FCF\Free\Integration\Integrator ) {
				...
				if ( version_compare( $version, '1000', '<=' ) ) {
					new Notice( __( "This version of ShopMagic for WooCommerce is not compatible with currently installed version of Flexible Checkout Fields. Please upgrade ShopMagic to the newest version.",
