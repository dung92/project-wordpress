...
class WoofiltersViewWpf extends ViewWpf {
	...
	public function renderHtml( $params ) {
		...
		if (isset($params['settings'])) {
			...
			$settings = $params;				
			DispatcherWpf::applyFilters('addDefaultFilterData', array(), $id, $settings);
			// other
		} else {
...
//		}
	...
	public function generateAttributeFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		if ($isCustom) {
			...
			$filterName     = "filter_{$filterNameSlug}_{$key}";
		}
	...
	private function generatePriceRangeOptionsHtml( $filter, $ranges, $layout ) {
		...
		foreach ($ranges as $range) {
			if ( !empty($range['1']) && isset($range['0']) ) {
				...
				// WOOCS plugin uses a different hook, use it if the standard one did not change the price
				$price_test = apply_filters( 'raw_woocommerce_price', $range[0] );
				if ( $price_test === $range[0] ) {
					$wc_price[0] = apply_filters( 'woocommerce_product_get_regular_price', $range[0] );
					$wc_price[1] = apply_filters( 'woocommerce_product_get_regular_price', $range[1] );
				} else {
					$wc_price = $range;
				}
