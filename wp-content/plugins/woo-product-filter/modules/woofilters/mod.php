...
class WoofiltersWpf extends ModuleWpf {
	...
	public function loadProductsFilter( $q ) {
		...
		if (ReqWpf::getVar('all_products_filtering')) {
			$exclude = array('paged', 'posts_per_page', 'post_type', 'wc_query', 'orderby', 'order', 'fields', 'product_cat');
	...
	public function getCurrencyPrice( $raw_price, $dec = false ) {
		...
		if ($price === $raw_price) {
			$price = apply_filters( 'woocommerce_product_get_regular_price', $raw_price );
		}
